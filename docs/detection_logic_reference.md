...
> **Cập nhật lần cuối:** 12-May-2026 (v6.5 — Raw Data + MS Risk Signals, no custom scoring)
> **Mục đích:** Tài liệu này mô tả chi tiết nghệp vụ và logic phân tích của `analyze_signins.py`.

> [!WARNING]
> **DEPRECATION NOTICE (v6.0–v6.5):** Các phần Scoring Matrix (Section 4), Verdict Classification (Section 5), và CSV Output Columns (Section 6) dưới đây đã bị **loại bỏ hoàn toàn** khỏi pipeline. Pipeline hiện tại chỉ hiển thị raw Microsoft Risk Signals (`RiskLevelDuringSignIn`, `RiskEventTypes`) — không có custom scoring, không có verdict, không có categorization. Các section này được giữ lại làm tài liệu lịch sử.

---

## 1. Nguồn Dữ Liệu Đầu Vào (Data Ingestion)

> **Nguyên tắc thiết kế (v6.5):** Tất cả KQL queries chỉ phục vụ **lấy raw data**. Pipeline Python chỉ **tổng hợp context** và **hiển thị MS Risk Signals** — không có custom scoring hay verdict.
...
*   **Hacker Botnet:** Đăng nhập từ **quốc gia lạ** (Foreign Country — không nằm trong TrustedCountries) **VÀ** thiết bị là thiết bị lạ (Unknown Device).
*   **VPN/Công tác hợp lệ:** Truy cập từ quốc gia lạ **NHƯNG** thiết bị sử dụng là **Trusted Device** (Tên máy tính quen thuộc).
*   **⚠️ Dedup (v3):** Nếu một quốc gia có **cả** sign-in Hacker (Unknown Device) **VÀ** sign-in VPN (Trusted Device), quốc gia đó **KHÔNG bị phạt**.

**Tầng 2 — Suspicious IP (Bất kể quốc gia — bắt hacker nội địa):**
*   Một IP được coi là **Suspicious** nếu nó là **Unknown IP** (không nằm trong TrustedIPs) **VÀ** **chưa từng xuất hiện cùng Trusted Device**.
...
Hệ thống tự động loại trừ các IP thuộc hạ tầng nội bộ của Microsoft trước khi tính anomaly.
...
## 4. ~~Hệ Thống Tính Điểm (Scoring Matrix)~~ — DEPRECATED (v6.0)

Điểm Anomaly Score được tính cộng dồn dựa trên các rủi ro sau (đã bị loại bỏ trong v6.0+):
...
| **AiTM Token Theft** | **+15 điểm** / mỗi high-risk session | Max 45đ | **[MỚI v5.0]** Multi-IP session + MFA bypass by token + Unknown Device. Migrated từ Q11 KQL |
| **Admin Account Boost** | **+10 điểm** | Tĩnh | Nếu tài khoản có quyền Admin và đã bị nghi ngờ (≥ 15đ) |

---

## 5. ~~Phân Loại Kết Quả (Verdict Classification)~~ — DEPRECATED (v6.0)
...
## 6. ~~CSV Output Columns~~ — DEPRECATED (v6.0)

File `investigation_report.xlsx` chứa các cột đáng chú ý:

| Cột | Mô tả |
|-----|--------|
| `TrustedCountries` | Danh sách countries thực sự Trusted (≥5% sign-ins). *Trước v3 bị ghi sai = AllCountries.* |
| `AllCountries` | Tất cả countries từng xuất hiện trong sign-in history (bao gồm cả hacker-generated) |
| `TrustedCountryCount` | Số lượng TrustedCountries (≥5%) — dùng cho BaselineWarning |
| `SuspiciousIPs` | Danh sách IP cụ thể gây ra DataBreach (Unknown IP + chưa từng dùng Trusted Device) |
...
| `AlertIPsMatched` | Danh sách Suspicious IPs trùng với Unfamiliar Sign-in alert IPs (Q00) |
| `AlertIPsMatchedCount` | Số lượng IPs trùng khớp (dùng cho scoring +3/IP) |
| `UserAlertCount_Q00` | Tổng số alerts từ Q00 cho user này |
| `FirstAlertTimestamp` | Thời điểm alert đầu tiên — dùng cho temporal baseline split |
| `CrossUserAlertIPs` | Alert IPs xuất hiện ở user KHÁC nhưng sign-in thành công ở user này — botnet indicator |
| `EffectiveThreshold` | Threshold thực tế dùng cho baseline (5% hoặc 15% cho low-volume users) |
| `AiTMSuspectedSessions` | **[MỚI v5.0]** Số sessions xuất hiện trên nhiều IPs (potential token theft) |
| `AiTMHighRiskSessions` | **[MỚI v5.0]** Số AiTM sessions có Unknown Device (high confidence) |
| `MfaSatisfiedByToken` | **[MỚI v5.0]** Số sign-ins MFA được bypass bằng token/PRT (AiTM indicator) |

> **Thay đổi v5.0 (11-May-2026):**
> - AiTM Token Theft detection migrated từ Q11 KQL vào Python `detect_aitm_sessions()`.
> - Q01A-F bổ sung cột `AuthenticationProcessingDetails` cho AiTM analysis.
> - Scoring: +15/high-risk AiTM session (multi-IP + unknown device + MFA by token), max 45đ.
