...
> **Cập nhật lần cuối:** 08-May-2026 (Logic Audit v3 — dedup scoring, CSV output fixes, KQL 11 toscalar fix)
...
> **Lưu ý (v3):** KQL 11 đã fix bug `toscalar()` — trước đây tính tổng sign-ins của TẤT CẢ users cộng lại làm mẫu số cho baseline 5%, gây ra baseline gần rỗng. Đã đổi sang `join` để tính `TotalPerUser` riêng từng user.
